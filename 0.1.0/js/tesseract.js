...
var defaultOptions = {
    ...
    //langPath: 'https://dirkncl.github.io/lang-tess/3.02/'
};
...
if (process.env.NODE_ENV === "development") {
    ...
    defaultOptions.workerPath = makeAbsolutePath(document.URL, './js/worker.js');
}
...
});function makeAbsolutePath(caller, relPath) { 
    var Caller = caller.split("/"); 
    var r_to_abs = relPath.split("/"); 
    Caller.pop();
    for (var i = 0; i < r_to_abs.length; i++) { 
        if (r_to_abs[i] == ".") 
            continue; 
        if (r_to_abs[i] == "..") 
            Caller.pop(); 
        else 
            Caller.push(r_to_abs[i]); 
  } 
  return Caller.join("/"); 
}
